...
public class CameraThrustOffset : MonoBehaviour
{
    ...
    [Tooltip("How far the camera pushes back when holding W.")]
    public Vector3 normalThrustOffset = new Vector3(0f, 0f, -2f);

    [Tooltip("How far the camera pushes back when boosting (Space).")]
    public Vector3 boostThrustOffset = new Vector3(0f, 0f, -6f);

    [Tooltip("How far the camera pushes FORWARD when reversing (S). Use positive Z values.")]
